...
export class RefreshAuthHandler implements ICommandHandler<RefreshAuthCommand> {
    ...
    async execute(command: RefreshAuthCommand): Promise<SignInAuthResult> {
        if (!command.refreshToken?.trim()) throw new UnauthorizedException('Refresh token missing');
        ...
        if (!userEntity || userEntity.id !== payload.sub) throw new UnauthorizedException('User no longer valid');
